<template>
  <div id="content">
    <div id="header">
      <h2><img :src="$store.state.user.user.data.icon" alt=""/></h2>
      <div class="user-box">
...
<script>
  ...
  export default {
    ...
    beforeRouteEnter(to,from,next){
      ...
      if(store.state.user.user.err === 0 && store.state.user.user.token.length >= 14){
        next()
      }else{
        next('/login')
      }
    ...
    methods:{
      logout(){
        ...
        this.$store.commit('user/UPDATE_USER',{err:1})
